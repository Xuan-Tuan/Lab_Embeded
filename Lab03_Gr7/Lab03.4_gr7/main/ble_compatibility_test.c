...
void example_exec_write_event_env(prepare_type_env_t *prepare_write_env, esp_ble_gatts_cb_param_t *param)
{
    ...
    {
        if (prepare_write_env->prepare_len == 4)
        {
            uint8_t write_data[4];
            memcpy(write_data, prepare_write_env->prepare_buf, 4);

            // Print the data received
            ESP_LOGI(EXAMPLE_TAG, "(3) ***** long write data received ***** \n");
            ESP_LOG_BUFFER_HEX(EXAMPLE_TAG, write_data, 4);
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_WRITE_EVT:
        if (!param->write.is_prep)
        {
            ...
            {
                uint8_t write_data[4] = {0x88, 0x99, 0xAA, 0xBB};
                memcpy(write_data, param->write.value, 4);
                ESP_LOGE(EXAMPLE_TAG, "(3) ***** write char1 ***** \n");
                esp_log_buffer_hex(EXAMPLE_TAG, write_data, 4);
                /* Xử lý in ra màn hình ở đây nha */
            }
